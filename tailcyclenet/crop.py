...
posetail 0.3.5 exposes the SAME rule as a public pure function (`posetail_dataset.crop_box_for_points`,
and `crop_cgroup_to_points*` call it). The local copy stays: it adds a `pad` parameter (0 for a
caller holding an already-padded stored extent) and returns None on an all-NaN input (the
detector depends on emitting a NaN box rather than raising). The base arithmetic is int32-exact
against the library, which `tests/test_dataset.py` asserts.
"""
import numpy as np
import torch
