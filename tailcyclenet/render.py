...
Body of `tailcyclenet render` (`tailcyclenet/__main__.py`) and of `scripts/render.py`
(`pixi run python scripts/render.py --pred pred/ --out clips/`) -- both call this module's
`main()` unchanged.
...
def build_parser() -> argparse.ArgumentParser:
    """The CLI parser for `tailcyclenet render` / `scripts/render.py`."""
    ap = argparse.ArgumentParser(description=__doc__,
                                 formatter_class=argparse.RawDescriptionHelpFormatter)
    ap.add_argument('--pred', required=True, type=Path,
                    help='a prediction SESSION directory from infer. An .npz is refused -- an '
                         'npz carries no provenance saying where its pixels are.')
...
def main(argv=None):
    ...
    An npz is refused because it carries no provenance: rendering it would mean restating the
    source root by hand, which is the exact failure this tool exists to avoid.
    ...
    auto-slices the camera axis on `overlay` and `boxes`. `load_predictions` is imported inside
    the call so importing this module for `render_group` alone does not pull the whole inference
    package.
    """
    import tomllib

    from .infer.predictions import load_predictions
    ...
    if args.pred.suffix == '.npz':
        raise SystemExit(
            ...
            'which is the exact failure this tool exists to avoid. `tailcyclenet eval` still '
            'scores this file directly; to look at it, re-run `tailcyclenet infer` from the run '
            'folder it names and render the session that produces.')
