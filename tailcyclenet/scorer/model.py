...
class PoseScorer(PoseTrackerEncoder):
    ...
    Adds, on top of `PoseTrackerEncoder`:
      ...
      * `score_feature`/`score_head` -- the scalar quality readout.
    ...
    def __init__(self, *args, pool_num_heads=8, score_hidden=64, use_precision=True, **kwargs):
        ...
        comes from there, so a warm start stays an exact load. The four keyword arguments here are
        the scorer's own heads and are FRESH at warm start.
        ...
        # Sized to the query encoder's OUTPUT dim, because it replaces the fused query embedding
        # before the decoder. Keeps missing slots finite so the decoder's temporal/camera
        # self-attention cannot be poisoned by a NaN -- the slot still flows through pooling with
        # its time embedding, so missingness informs the score instead of being hidden from it.
    ...
    def _scene_scalars(self, coords_flat, camera_group, device, times=None):
        ...
        `coords_flat`: [B, T*K, R]. Mirrors `TrackerEncoder.forward` so the numbers match what the
        warm-started weights were trained under, with the query axis flattened over (t, k) instead
        of (t, n) -- same thing, different name. Computed from the RAW NaN-carrying coords:
        `get_camera_scale` drops NaN via `is_point_visible` and `scene_center` uses `nanmean`, so
        missing slots cannot bias the scene scale or centroid.
        """
        B, _, R = coords_flat.shape
        n_cams = len(camera_group)

        if R == 3:
            cube_scale = get_camera_scale(camera_group, coords_flat, times=times)  # (n_cams, B)
        else:
    ...
    def _fill_nearest_valid(coords, valid):
        ...
        `coords`: [b, t, k, R]; `valid`: [b, t, k]. Purely to keep projection and rays finite --
        the token below is what the decoder and pooling actually see for those slots, and a NaN in
        a masked key would still poison the decoder's self-attention. An all-missing point falls
        through as 0 after the caller's `nan_to_num`.
        """
    ...
    def _kpt_ids_checked(self, kpt_ids, n_kpt):
        ...
        The scorer is conditioned on the keypoint REGISTRY, so scoring a root whose names were
        not in the training registry is out of range -- refused by name rather than by an index
        error deep inside an embedding lookup.
        """
    ...
    def score(self, views_norm, scene_features, coords_full, camera_group, kpt_ids,
        ...
        # Temporal-RoPE key positions for the scene tokens. The decoder REQUIRES these when
        # `cross_attn_rope` is on, and the token count is derived from the tokens the encoder
        # actually produced rather than recomputed from T (at T < tubelet_size the latter is 0,
        # which would yield zero scene tokens). Computed once -- it depends on the scene, not on
        # the keypoint slice.
        ...
        if self.cross_attn_rope:
            ...
            scene_frame_pos = slot.float() * tub + (tub - 1) / 2.0

        def _score_slice(cf, valid_s, occ_s, k0):
            Kc = cf.shape[2]
            self.query_encoder._kpt_ids = kpt_ids[:, k0:k0 + Kc]
            self.query_encoder._query_ok = torch.ones((B, Kc), dtype=torch.bool, device=device)
            coords_flat = rearrange(cf, 'b t k r -> b (t k) r')

            # QUERY == TARGET: each (t, k) token lives at its own frame, evaluated at that frame.
            ...
            # Missing slots get the learned token; their sanitised coords only kept projection and
            # rays finite.
            query_embeds = torch.where(
                (~valid_s)[..., None, None], self.missing_point.to(query_embeds.dtype), query_embeds)

            if R == 3:
                p2d_query = project_points_torch(camera_group, query_coords)
            ...
            # Pool only OBSERVED (t, cams) slots, so missingness cannot shift the score -- it is
            # orthogonal to track quality. True == drop. Force-unmask defensively: a softmax over
            # all -inf is NaN, and `min_valid_frames` is a training-side guarantee, not something
            # this function should assume of its caller.
        ...
        # Set immediately before the `try` and cleared in its `finally`: an exception anywhere
        # after this point must not leave a stash behind, or the NEXT window's forward silently
        # runs with this window's ids. `_box_prompt` is never a real box -- `box_prompt = "film"`
        # keeps the model's `missing_film` token in place, so the warm start is an exact load and
        # a future box-at-QC-time arm stays buildable.
        ...
        try:
            if kpt_chunk and K > kpt_chunk:
                ...
                for k0 in range(0, K, kpt_chunk):
                    ...
                    s, p = _score_slice(coords_full[:, :, k0:k1], valid[:, :, k0:k1], occ, k0)
                    s_parts.append(s)
                    p_parts.append(p)
                return torch.cat(s_parts, dim=1), torch.cat(p_parts, dim=1)
            return _score_slice(coords_full, valid, occlusion, 0)
        finally:
            # Always cleared: a leaked stash applies one window's ids to the next, which at
            # batch_size 1 is a silent wrong answer rather than a shape error.
    ...
    def forward(self, views, coords, camera_group, kpt_ids, kpt_chunk=None, occlusion=None):
        """Single-sample inference path. `views`: list of [b,t,h,w,c] uint8 or float;
        `coords`: [b,t,k,R]; `kpt_ids`: [b,k] global registry ids (see `score`).
        Returns (scores, precision), each [b, k]."""
    ...
    def score_triplet(self, trip):
        ...
        `trip` is the dict from `tailcyclenet.scorer.triplet.make_triplet`. Returns
        `scores, precision, labels`, each [N, 3], N = b*k, columns (good, bad, anchor).
        """
        gv, gc, gcg = trip['good']
        _, bc, bcg = trip['bad']                       # bad shares good's pixels by construction
...
def build_scorer(model_cfg: dict, n_keypoints: int, **scorer_kwargs) -> PoseScorer:
    ...
    `scorer_kwargs` are the scorer's own head settings (`pool_num_heads`, `score_hidden`,
    `use_precision`); they are FRESH parameters at warm start, so they are not part of the shape
    contract that must match a pose checkpoint.
    """
